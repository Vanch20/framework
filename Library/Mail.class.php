 ...
 *
 * 用法:
 * $mail = new Mail('SMTP服务器地址', '用户名', '密码');
 * $mail->send('收件人地址', '内容', '标题');
 */
class Mail
{
	/**
	 ...
	 * 发送邮件
	 *
	 * @param string $sMail 收件人地址
	 * @param string $sContents 邮件内容
	 * @param string $sSubject 邮件标题
	 * @return boolean
	 */
	public function send($sMail, $sContents, $sSubject = "")
	{
		if (strpos($sMail, '@') == 0)
		{
			$this->_error = "电子邮件地址不正确";
			return false;
		}

		if (empty($sContents))
		{
			$this->_error = "电子邮件内容为空";
			return false;
		}

		if (empty($sSubject))
		{
			$sSubject = '=?utf-8?B?'.base64_encode('').'?=';
		}
		else
		{
		...
		$smtp = new smtp($this->_server, 25, true, $this->_user, $this->_pass);
		$smtp->debug = false;
		$r = $smtp->sendmail($sMail, $this->_user, $sSubject, $sContents, $this->_type);
		unset($smtp);
